...
static void calc_quad(sp_node *node, sp_rect rects[SP_NODE_CHILD_MAX]) {
...
static void calc_split(sp_node *node, sp_rect rects[SP_NODE_CHILD_MAX]) {
...
sp_node *sp_new_leaf(sp_init_fn *fn1, sp_resize_fn *fn2, sp_free_fn *fn3) {
    assert(fn1 && fn2 && fn3);
